...
def read_grid(file_path):
    ...
    # For each line in the file, create a list of integers, and then append
    # that list of integers to pixel_grid to make it a row of a grid.
    for line in my_file:
        int_list = csv_line_to_list(line)
        pixel_grid.append(int_list)
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    # This for loop will will loop over all 9 possible pixels by finding their
    # corresponding indexes, and at each possible pixel value we will use the
    # get_pixel_at to get the value at that pixel, then add it to our sum.
    for k in range(i - 1, i + 2):
        for n in range(j - 1, j + 2):
            pixel_sum += get_pixel_at(pixel_grid, k, n)
...
def blur(pixel_grid):
    ...
    # This for loop will loop through all the possible rows, and append an
    # empty sub-list to blurred_grid. The bested loop will loop through each
    # column and will add the blurred values of pixel_grid to their
    # corresponding position in blurred_grid.
    for row_index in range(len(pixel_grid)):
        blurred_grid.append([])
        for col_index in range(len(pixel_grid[0])):
            blurred_grid[row_index].append(average_of_surrounding(pixel_grid,
                                                                  row_index,
                                                                  col_index))
...
blurred_input = blur(input_grid)

# Step E: Write the result to both output files

write_image(out_image_filename, blurred_input)
write_grid(out_grid_filename, blurred_input)
...
# For this assignment, I briefly collaborated with Tanner Huck by offering him
# suggestions on how to make some of his functions work more efficiently.
