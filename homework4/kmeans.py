...
def euclidean_distance(point1, point2):
    ...
    # Since we can assume that both the points will be in the same dimension,
    # len(point1) = len(point2). So it'd be correct to use either in this case.
    total = 0
    for index in range(len(point1)):
        total += (point2[index] - point1[index]) ** 2
...
def get_closest_centroid(point, centroids_dict):
    ...
    # For each key in the passed in dictionary, find the value associated with
    # that key. Then find the distance between the passed in data point and
    # the given value from the dictionary. Repeat this with every value until
    # you find the centroid with the smallest distance between the data point.
    count = 0
    short_dist = 0
    curr_cent = ""
    for key, value in centroids_dict.items():
        curr_dist = euclidean_distance(point, value)
        if count == 0 or curr_dist < short_dist:
...
def update_assignment(list_of_points, centroids_dict):
    ...
    # already in the dictionary, create the key. If the centroid name is
    # already in the dictionary, append the new list to the existing list.
    new_cent_dict = {}
    for element in list_of_points:
        ...
        if cent_of_point not in new_cent_dict:
            new_cent_dict[cent_of_point] = [element]
        else:
            new_cent_dict[cent_of_point].append(element)
...
def mean_of_points(list_of_points):
    ...
    # Find the dimension of each data point, then find how many data points
    # there are in total. Then sum together the values of the nth entry of each
    # data point. Then divide this total by the total number of data points
    # added to find the mean.
    list_of_means = []
    for j in range(len(list_of_points[0])):
        total = 0
        for i in range(len(list_of_points)):
            total += list_of_points[i][j]
        list_of_means.append(total / len(list_of_points))
...
def update_centroids(assignment_dict):
    ...
    # For each key in the passed in dictionary find the value associated with
    # that key. Then find the mean list of the data point list, and add it to
    # the new dicitonary.
    mean_cent_dict = {}
    for key, value in assignment_dict.items():
        mean_cent_dict[key] = mean_of_points(value)
    return mean_cent_dict
